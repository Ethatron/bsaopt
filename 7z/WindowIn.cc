...
HRESULT CIn::ReadBlock()
{
  ...
  while(true)
  {
    INT aSize = (INT)((m_BufferBase + m_BlockSize) - (m_Buffer + m_StreamPos));
    if(aSize == 0)
      return S_OK;
    INT aNumReadBytes;
    ...
    if(aNumReadBytes == 0)
    {
      ...
      if(aPointerToPostion > m_PointerToLastSafePosition)
        m_PosLimit = (INT)(m_PointerToLastSafePosition - m_Buffer);
...
{
  BeforeMoveBlock();
  INT anOffset = (INT)((m_Buffer + m_Pos - m_KeepSizeBefore) - m_BufferBase);
  INT aNumBytes = (INT)((m_Buffer + m_StreamPos) -  (m_BufferBase + anOffset));
